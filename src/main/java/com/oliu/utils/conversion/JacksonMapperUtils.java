package com.oliu.utils.conversion;
 ...
 */
public class JacksonMapperUtils extends ObjectMapper {

    private static Logger logger = LoggerFactory.getLogger(JacksonMapperUtils.class);

    private static JacksonMapperUtils mapper;

    public JacksonMapperUtils() {
        this(null);
    }
     ...
     * @param include
     */
    public JacksonMapperUtils(Include include) {
        //设置输出时包含属性的风格
        if (include != null) {
     ...
     */
    public static JacksonMapperUtils getInstance() {
        if (mapper == null) {
            mapper = new JacksonMapperUtils();
        }
        return mapper;
    }
     ...
     * 创建只输出非Null且非Empty(如List.isEmpty)的属性到Json字符串的Mapper,建议在外部接口中使用.
     */
    public static JacksonMapperUtils nonEmptyMapper() {
        return new JacksonMapperUtils(Include.NON_EMPTY);
    }

    /**
     * 创建只输出初始值被改变的属性到Json字符串的Mapper, 最节约的存储方式，建议在内部接口中使用。
     */
    public static JacksonMapperUtils nonDefaultMapper() {
        return new JacksonMapperUtils(Include.NON_DEFAULT);
    }
     ...
     * 允许不带引号的字段名称
     */
    public JacksonMapperUtils enableSimple() {
    ...
    public <T> T updateJsonBean(String jsonString, T object) {
        try {
            return (T) this.readerForUpdating(object).readValue(jsonString);
        } catch (JsonProcessingException e) {
            logger.warn("updateJsonBean json string:" + jsonString + " to object:" + object + " error.", e);
        } catch (IOException e) {
            logger.warn("updateJsonBean json string:" + jsonString + " to object:" + object + " error.", e);
        }
        return null;
     ...
     */
    public JacksonMapperUtils enableEnumUseToString() {
